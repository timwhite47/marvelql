...
var cacheCollection = exports.cacheCollection = function cacheCollection(key, fetcher) {
  var offset = 0;

  var limit = API_LIMIT;
  return cacheFetch(key, function () {
    return fetcher({ limit: limit, offset: offset }).then(function (_ref) {
      var meta = _ref.meta;

      var pages = (0, _lodash.range)(meta.total / API_LIMIT);

      return _bluebird2.default.map(pages, function (pageNumber) {
        offset = pageNumber * API_LIMIT;

        return fetcher({ offset: offset, limit: limit }).then(_api_helpers.parseCollection);
      });
    });
